...
import {
  ...
  RECIPE_IN_CATEGORY_FEATURE_COUNT,
} from "../../constants";
      ...
      try {
        ...
        if (config) {
          ...
          if (configEventRightActive) {
            ...
            if (
              configEventRightVideo &&
              configEventRightActive === EVENT_HOME_BY.VIDEO
            ) {
            ...
            if (
              configEventRightBanner &&
              configEventRightActive === EVENT_HOME_BY.BANNER
            ) {
    ...
    resolve: async (_, args, context, info) => {
      ...
      try {
        ...
        while (indexId < idWhereClause.length) {
          const cate = await CategoryTC.getResolver(RESOLVER_FIND_ONE).resolve({
            args: {
              filter: {
      ...
      try {
        ...
        if (config) {
          ...
          if (
            ...
            if (configBrand1 && configBrand1.key) {
              idWhereClause.push(configBrand1.key);
            }
            if (configBrand2 && configBrand2.key) {
              idWhereClause.push(configBrand2.key);
            }
            if (configBrand3 && configBrand3.key) {
              idWhereClause.push(configBrand3.key);
            }
            if (configBrand4 && configBrand4.key) {
              idWhereClause.push(configBrand4.key);
            }

            if (configBrand5 && configBrand5.key) {
              idWhereClause.push(configBrand5.key);
            }

            if (configBrand6 && configBrand6.key) {
              idWhereClause.push(configBrand6.key);
            }
          }
        }
